...
if(isset($_POST['login'])) {
    ...
    if (empty($eml) && empty($pass)) {
        $message = "<p id='mssg2'>Please fill in the username and code fields *</p>";
    } elseif (empty($eml) && !empty($pass)) {
        $message1 = "<p id='mssg'>Please fill in the username field *</p>";
    } elseif (!empty($eml) && empty($pass)) {
        $message2 = "<p id='mssg'>Please fill in the code field *</p>";
    } else {
        $stmt = $pdo->prepare("SELECT * FROM admin WHERE code = :pass AND username = :eml");
        $stmt->execute(array(':pass' => $pass, ':eml' => $eml));
        $rows = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$rows) {
            $message = "<p id='mssg2'>Incorrect username or code</p>";
        } else{
            echo'
            <form class="login-form" id="robot">
                <div class="dd">
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <div class="form-floating">
                                <input type="text" class="form-control" id="email" placeholder="example@gmail.com" name="eml">
                        ...
                        <div class="col-12">
                            <div class="form-floating">
                                <input type="password" class="form-control" id="subject" placeholder="Password" name="pass">
                                <label for="subject">Code Admin</label>
                            </div>
                            <p>
                                <?php
                                if(isset($message2)){
...
/* i am not a robot */
#robot{
    background-color: rgba(159, 162, 161, 0.708);
    padding: 10px 50px;
    width: fit-content;
    border-radius: 5px;
...
.dd{
    background-color: rgba(39, 82, 225, 0.906);
    padding: 5px;
    border-radius: 10%;
    align-items: center;
}
